...
// cache commonly used elements, so we don't hit the DOM all the time
var theTime, scrambleEl;
...
// deal with styles
var styleName = "css/style" + getCookie("style", 0) + ".css";
...
function initialize(lookForTimes, checkQueryString) {
 ...
 $('toggler').innerHTML = (timerupdate==0) ? "off" : (timerupdate==1) ? "on" : "seconds only";
 useMilli = getCookie("useMilli", false);
 $('millisec').innerHTML = (useMilli) ? "1\/1000 sec" : "1\/100 sec";
 var oldManualEnter = manualEnter;
 manualEnter = getCookie("manualEnter", false);
 ...
 scrambleSize = (a=getCookie("scrSize", ""))==="" ? 16 : parseInt(a,10);
 scrambleEl.style.fontSize = scrambleSize + "px";
 ...
 useInspection = getCookie("inspection", false);
 $('inspec').innerHTML = (useInspection) ? "WCA" : "no";
 if (!useInspection) { useBld = getCookie("useBld", false); }
 else { useBld = false; setCookie("useBld", false); }
 $('bldmode').innerHTML = (useBld) ? "on" : "off";
 useAvgN = getCookie("useAvgN", 0);
 $('avgn').innerHTML = (useAvgN==1) ? "using" : "not using";
 useMoN = getCookie("useMoN", false);
 $('mon').innerHTML = (useMoN) ? "using" : "not using";
 useMono = getCookie("useMono", 0);
 $('monospace').innerHTML = (useMono==1) ? "on" : "off";
 scrambleEl.style.fontFamily = (useMono==1) ? "monospace" : "serif";
...
function startTimer(keyCode) {
 if (timerStatus == 0 && !manualEnter && keyCode == 32 && importFocus == 0) {
  timerStatus = 3;
 } else if (timerStatus == 3 && !manualEnter && keyCode == 32 && (new Date()).getTime() - curTime >= 300 && importFocus == 0) {
  if (type=="sqrs") { scrambleEl.innerHTML = "scramble: loading... "; }
  if (useInspection) {
   timerStatus = 2;
   ...
   timerStatus = 4;
   memoTime = new Date().getTime();
   theTime.style.color = $('memcol').value;
   if (timerupdate > 0) {memoID = setInterval(updateMemo, (timerupdate==1)?10:100);}
   ...
   if (timerupdate > 0) {timerID = setInterval(updateTimer, (timerupdate==1)?10:100);}
...
function stopTimer(keyCode) {
 ...
 if (manualEnter) {
  ...
  if (timerupdate > 0) {clearInterval(timerID);}
  getTime(penalty);
  scrambleArr[scrambleArr.length] = scramble;
  rescramble3();
 }
...
function loadList() {
 ...
 $('theList').scrollTop = $('theList').scrollHeight;
 changeColor();
}
...
function getlastscramble() {
 scrambleEl.innerHTML = "scramble: " + scramble + "<br> last scramble: " + lastscramble;
 $('getlast').innerHTML = "";
}
...
function toggleBld() {
 if (!useInspection) { useBld = !useBld; }
...
function toggleInput() {
 ...
 $('inputTimes').innerHTML = (manualEnter) ? "typing" : "timer";
 theTime.innerHTML = (manualEnter) ?
...
function toggleInspection() {
 ...
 if (useInspection) { useBld = false; }
 setCookie("useBld", useBld);
 setCookie("inspection", useInspection);
 $('inspec').innerHTML = (useInspection) ? "WCA" : "no";
 $('bldmode').innerHTML = (useBld) ? "on" : "off";
}
...
function getAvgSD(start, nsolves, SD) {
 ...
 for (j=0; j<nsolves; j++) {
  ...
  t = (useMilli ? 10*t : 10*Math.round(t));
...
var scrdata = [["===WCA PUZZLES===",[["--","blank",0]]],
 ...
 ["3x3x3 subsets",[["2-generator <R,U>","2gen",25],["2-generator <L,U>","2genl",25],
  ...
  ["corners only","corners",0],["last layer","ll",0],["last slot + last layer","lsll2",0],
...
function scrambleIt(){
 ...
 scramble = ss[0];
 scrambleEl.innerHTML = "scramble: " + scramble + "&nbsp;";
}
